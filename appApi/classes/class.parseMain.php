...
{	
    ...
    function _userextraAdd($requestElemets){
		   ...
		   if($res !=0)
			{   
				...
				if(isset($requestElemets["sendHistory"])){
				$value["send_history"] = $this ->com->valueChecking($requestElemets["sendHistory"]);}
    ...
    function _increment($requestElemets){ 
	          ...
	          if(!array_key_exists('tableName',$requestElemets)){
				      $res=0;
				      $outPut['STATUSCODE']	=	400;
			          $outPut['MESSAGE']    =	"param is missing for tableName";
					  return $outPut;
				   }
				   if(!array_key_exists('recordId',$requestElemets)){
				      $res=0;
				      $outPut['STATUSCODE']	=	400;
			          $outPut['MESSAGE']    =	"param is missing for recordId";
					  return $outPut;
					  }
