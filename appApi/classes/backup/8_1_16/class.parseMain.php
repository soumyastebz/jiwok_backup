...
{	
    ...
    function _userextraAdd($requestElemets){
		      global $res;// this value will change to 0 if any error occur
	           if(!array_key_exists('userid',$requestElemets)){
				      $res=0;
					  $val = "No values found for userid";
		              $error_message['error']	=	$val ; 
			  ...
			  $res=0;
			  $val = "Expecting  numeric values for  userid";
		      $error_message['error']	=	$val ; 
		      return $error_message;
			  }else{ 
		   ...
		   if($res !=0)
			...
			if($tble_ins){
				    ...
				    return $message;	
					     ...
					     $res = 0;
						 $val = "Data is not inserted ,please try again";
						 $error_message['error']	=	$val ; 
						 return $error_message;	
			       }
		  }
   ...
   function _increment($requestElemets){ 
	          ...
	          if(!array_key_exists('tableName',$requestElemets)){
				      $res=0;
					  $val = "No values found for tableName";
		              $error_message['error']	=	$val ; 
					  return $error_message;
				   }
				   if(!array_key_exists('recordId',$requestElemets)){
				      $res=0;
					  $val = "No values found for recordId";
		              $error_message['error']	=	$val ; 
					  return $error_message;
				   }
	            ...
	            $result    = end($statement->fetch());
					if($result>0){
						   if(!in_array($downloads, array('1','-1'), true ))
							  { 
								  $res = 0;
								  $val = "Expecting  digit 1 or -1 values for downloads";
								  $error_message['error']	=	$val ; 
								  return $error_message;
							  }
				  ...
				  if($result>0){
						 ...
						 $res = 0;
						 $val = "recordId/userid is not found on table";
						 $error_message['error']	=	$val ; 
						 return $error_message;	
					}
				  break;
			...
			if($tble_ins){
				    ...
				    return $message;	
					     ...
					     $res = 0;
						 $val = "Data is not updated ,please try again";
						 $error_message['error']	=	$val ; 
						 return $error_message;	
			       }
		   }
