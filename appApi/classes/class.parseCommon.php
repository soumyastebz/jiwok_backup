<?php /************************************************************ 
   Project Name	::> jiwok App Benoit   
   Module 		::> Class for db  purpose
   Programmer	::> Soumya/Georgina
   Date			::> 15-07-2015
   DESCRIPTION::::>>>>
   Class for db purpose
*************************************************************/
...
{  /*
	...
	*/
	...
	public function valueChecking($value){
			...
			//return mysql_real_escape_string(trim(urldecode($value)));
	} 
   ...
   //insertion to another db for log userevent
   public function insertRecord_logdb($table,$data)
    {
    // try our best not to use reserved words
    $columns = array_map(function($column) {
        return "`$column`";
    ...
    $sql = sprintf('INSERT INTO `%s` (%s) VALUES (%s)', $table,join(',', $columns),join(',', $values));
    // prepare and execute it
    $stmt = $GLOBALS['db_logtype']->prepare($sql);
    $stmt->execute(array_values($data));
    $val  = $stmt->rowCount();
    return $val ? 1 : 0;
   }
