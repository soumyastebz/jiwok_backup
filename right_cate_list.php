   ...
   //foreach($getAllTrainCats as $i=>$val){
	   	...
	   	foreach($colArray as $col){
			...
			if($getAllTrainCats[$i]['url']=='')
               ...
               <li> <a id="topicName" href="javascript:void(0)" onClick="showhide('itoms_0<?=$i;?>');" class="itom_s has-sub"><?php echo ucfirst(htmlentities($getAllTrainCats[$i]['category_name'], ENT_COMPAT, 'UTF-8')); ?></a></li>
                
                
                
                <ul class="expand" id="itoms_0<?=$i;?>">
			...
			?>
				...
				{ ?>
					...
					<li><a id="topicName" class="itom_s" href="<?=ROOT_JWPATH.$getAllTrainCats[$i]['url']?>"><?php echo ucfirst(htmlentities($getAllTrainCats[$i]['category_name'], ENT_COMPAT, 'UTF-8')); ?></a></li>
