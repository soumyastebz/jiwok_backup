<?php
// Code to redirect from beta.jiwok.com   to www.jiwok.com
/*if($_SERVER['HTTP_HOST'] == "beta.jiwok.com"){
	$currUrl=(!empty($_SERVER['HTTPS'])) ? "https://www.jiwok.com".$_SERVER['REQUEST_URI'] : "http://www.jiwok.com".$_SERVER['REQUEST_URI'];
...
if(strtolower($domain)	==	'www'){//If jiwok.com then both languages should be available pl,en and fr
	...
	if(in_array(strtolower($domain),$checkDomain)){//Check If brands parismarathon or semideparis
		...
		if(strtolower($domain)	!=	'www'){	
			...
			if($matchEn)$_SESSION['language']['langId']	=	1;

			if($matchEs)$_SESSION['language']['langId']	=	3;
			if($matchPl 	== 1)	$_SESSION['language']['langId'] = 5;//If Match then lang = pl 
			if($_SESSION['language']['langId'] == ''){//If lang not Set
...
/*________________________ASSIGNMENT THE SESSION VALUES FOR PAGE VARIABLES_______________*/
	...
	if($_SERVER['HTTP_HOST'] == "beta.jiwok.com"){
		...
		$urlvar	=	"https://".$_SERVER['HTTP_HOST']."/";
