<?php
   ...
   // include_once('mes_historical.php');
	...
	/* Validation for add and update*/
		...
		if($errorMsg==0){
				...
				if($_POST['update']){ 
                    ...
                    if($_FILES['user_photo']['name'] != ""){
                            ...
                            $nextUpload = $objGen->_fileUploadWithImageResize('user_photo',ROOT_FOLDER.'uploads/users/',$fileName,166,145);
							$fileName = $fileName.".".$extension;
							$img_file = "ph_".$fileName;
							$thumb_file = "th_".$fileName;
							$thumbUpload = $objGen->_fileImageResize2($img_file,ROOT_FOLDER.'uploads/users/',$thumb_file,87,105);
                            $_POST['user_photo']    = $fileName;
							if($_POST['user_photo'] !="" && is_file(ROOT_FOLDER."uploads/users/".$_POST['current_photo'])){ 
							unlink(ROOT_FOLDER."uploads/users/".$_POST['current_photo']);
							unlink(ROOT_FOLDER."uploads/users/th_".$_POST['current_photo']);
							unlink(ROOT_FOLDER."uploads/users/ph_".$_POST['current_photo']);
							}
	 ...
	 if($currentImage != ""){
        $imageDetails = getimagesize(ROOT_FOLDER.'uploads/users/'.$currentImage);
    }
...
<style>
.has-js .label_check, .has-js .label_radio {  display:inline-block;background-position: 0 0px;}
<!--res-->		
	.simplePopup {		
	display:none;		
	position:fixed;		
...
<!--res-->
<!--
<script src="<?=ROOT_FOLDER?>js/flaunt.js" type="text/javascript"></script>
-->
    <div class="frame_inner">
       <section class="profile">
         <div class="left">
               <ul class="bredcrumbs">
                <li style="color: #1473B4; font-weight: 700;"><?=mb_strtoupper($parObj->_getLabenames($arrayDataProfile,'newPgeTxt','name'),'UTF-8');?> :</li>
                <li><a href="<?=ROOT_JWPATH?>index.php"><?=mb_strtoupper($parObj->_getLabenames($arrayDataProfile,'newHmeTxt','name'),'UTF-8');?></a></li>
                <li style="color: #1473B4; font-weight: 700;">></li>
               <li ><a href="#" class="select" style="color: #E67F23;"><?=mb_strtoupper($parObj->_getLabenames($arrayDataProfile,'newHeadTxt','name'),'UTF-8');?></a></li>
            </ul>
            <!--crop-->
             <? if($currentImage){?>
<!--
...
-->
					    <img src='<?=ROOT_FOLDER?>uploads/users/<?=$currentImage?>'>
					  <?} else
					{?>
			
				<img src="<?=ROOT_FOLDER?>images/profile-dummy.png" alt="profile image">
				
        <? }?>
<!--
       <input type="button"   class="show1" value="Crop" >
-->
   <a href="#"  class="show1">Crop</a>
        </figure></div>
...
" >
  ...
  <link href="<?=ROOT_FOLDER?>crop/assets/css/bootstrap.min.css" rel="stylesheet">
  <link href="<?=ROOT_FOLDER?>crop/assets/css/cropper.min.css" rel="stylesheet">
  <style>
  ...
  .se-pre-con {
    ...
    height: 100%;
    z-index: 9999;
    background: url("<?php echo JIWOK_URL; ?>crop/assets/img/preloader.gif") center no-repeat #fff;
  }
  .se-pre-upload {
    position: fixed;
    left: 0px;
    ...
    height: 100%;
    z-index: 9999;
    background: url("<?php echo JIWOK_URL; ?>crop/assets/img/upload-preloader.gif") center no-repeat rgba(255, 255, 255, 0.9);
  }
...
-->
      ...
      require_once 'crop/config.php';
       ...
       $imagePath = ROOT_FOLDER.'uploads/users/';
      //neethu
    ...
    <input type="button" id="cropit1" value="Crop image!" style="background: none repeat scroll 0 0 #E68023;
    border: 2px solid #E68023;
    color: #FFFFFF;
    font-size: 16.39px;
    ">
<!--
  ...
  <div>
   ...
   <!----cropping code goes here-->
         ...
         <? if($currentImage=="" ){?>
			 ...
			 <img src="<?=ROOT_FOLDER?>images/profile-dummy.png" alt="profile image"></figure><?}?>
<!--
         </form>
-->
         <!--crop-->
...
-->
          <h2 class="name"><?=stripslashes(mb_strtoupper($_POST['user_fname']))?> <?=mb_strtoupper(stripslashes($_POST['user_lname']))?></h2>
        ...
        <p class="title3"> </p>
          ...
          <div class="bloks">
           <div class="rows"><? if($err0 == 1){ ?><div class="prf_err"> <img src="<?=ROOT_FOLDER?>images/help_editprfl.png" class="Q-mark"   alt="help" onMouseOver="tooltip('<?=$emailMsg?>');" onMouseOut="exit();"><?}?>
				  
               <div class="col-01" ><?=mb_strtoupper($parObj->_getLabenames($arrayData,'email','name'),'UTF-8');?>:</div>
               <div class="col-02 "><input name="user_alt_email" type="text" style="color: #408DC1;" class="tfl" value="<?=stripslashes($_POST['user_alt_email'])?>"></div>
               
              <? if($err0 == 1){ ?></div><?}?>
            </div>
      
             <div class="rows"> <? if($err1 == 1){ ?><div class="prf_err"><img src="<?=ROOT_FOLDER?>images/help_editprfl.png" class="Q-mark"  onMouseOver="tooltip('<?=$fname?>');" onMouseOut="exit();"><? }?>
               <div class="col-01"><?=mb_strtoupper($parObj->_getLabenames($arrayData,'fname','name'),'UTF-8');?>:</div>
               <div class="col-02"><input name="user_fname" style="color: #408DC1;" type="text" class="tfl" value="<?=stripslashes($_POST['user_fname'])?>"></div>
                <? if($err1 == 1){ ?> </div><?}?>
            </div>
            
           <div class="rows">  <? if($err2 == 1){ ?><div class="prf_err"><img src="<?=ROOT_FOLDER?>images/help_editprfl.png" class="Q-mark"  onMouseOver="tooltip('<?=$lname?>');" onMouseOut="exit();"><? }?>
               <div class="col-01" <? if($err2 == 1){ ?> <? }?>><?=mb_strtoupper($parObj->_getLabenames($arrayData,'lname','name'),'UTF-8');?>:</div>
               <div class="col-02"><input name="user_lname" style="color: #408DC1;" type="text" class="tfl" value="<?=stripslashes($_POST['user_lname'])?>"></div>
          <? if($err2 == 1){ ?> </div><?}?>
            </div>
            
            <div class="rows"><? if($err3 == 1){ ?><div class="prf_err"><img src="<?=ROOT_FOLDER?>images/help_editprfl.png" class="Q-mark"   onMouseOver="tooltip('<?=$gender?>');" onMouseOut="exit();"><? }?>
               <div class="col-01" ><?=mb_strtoupper($parObj->_getLabenames($arrayData,'gender','name'),'UTF-8');?>: </div>
             ...
             <div class="rows">  <? if($err4 == 1){ ?><div class="prf_err"><img src="<?=ROOT_FOLDER?>images/help_editprfl.png" class="Q-mark" onMouseOver="tooltip('<?=$dob?>');" onMouseOut="exit();"><?}?>
                  <div class="col-01"><?=mb_strtoupper($parObj->_getLabenames($arrayData,'dob','name'),'UTF-8');?>:</div>
               <div class="col-02"> <div class="split nais">
                     <div class="selet3 nais3 frst-nais">
				...
				?>
                </select> </div></div> <div class="split nais">
                     <div class="selet3 nais3">
           ...
           <div class="rows"> <? if($err10 == 1){ ?><div class="prf_err"><img src="<?=ROOT_FOLDER?>images/help_editprfl.png" class="Q-mark"  onMouseOver="tooltip('<?=$country_err_msg?>');" onMouseOut="exit();"><? }?>
               <div class="col-01" ><?=mb_strtoupper($parObj->_getLabenames($arrayData,'country','name'),'UTF-8');?>:</div>
            ...
            <div class="rows"><? if($err22 == 1){ ?><div class="prf_err"><img src="<?=ROOT_FOLDER?>images/help_editprfl.png" class="Q-mark"  onMouseOver="tooltip('<?=$timezone_err_msg?>');" onMouseOut="exit();"><? }?>
               <div class="col-01" ><?=mb_strtoupper($parObj->_getLabenames($arrayData,'timezone','name'),'UTF-8');?>:</div>
               <div class="col-02"><div  class="selet3"><select  name="user_timezone" id="user_timezone" >
                  <?php foreach($time_zones as $time_zone){ ?>
					...
					<option value="<?php echo $time_zone['time_tz']; ?>" <? if($_POST['user_timezone']== $time_zone['time_tz']) print 'selected';?>><?php if($lanId==2){echo $time_zone['gmt_timezone'];}else{echo $time_zone['time_name'];}?></option><?php } ?>
                       </select></div></div><? if($err22 == 1){ ?> </div><?}?>
            </div>
           
             <div class="rows">	<? if($err12 == 1){ ?><div class="prf_err"><img src="<?=ROOT_FOLDER?>images/help_editprfl.png" class="Q-mark"  onMouseOver="tooltip('<?=$language?>');" onMouseOut="exit();"><? }?>
               <div class="col-01" ><?=mb_strtoupper($parObj->_getLabenames($arrayData,'languagepreffer','name'),'UTF-8');?>: </div>
               <div class="col-02"><div  class="selet3"><select name="user_language">
				  ...
				  foreach($siteLanguagesConfig as $key=>$data){
					...
					<option  value="<?=$key?>" <? if($_POST['user_language']==$key) print 'selected'; if($_POST['user_language']=='' && 
                       ...
                       </select></div></div><? if($err12 == 1){ ?> </div><?}?>
            </div>

 <div class="rows"><? if($err13 == 1){ ?><div class="prf_err"><img src="<?=ROOT_FOLDER?>images/help_editprfl.png" class="Q-mark"  onMouseOver="tooltip('<?=$photo?>');" onMouseOut="exit();"><? }?>
         <div class="col-01"> <?=mb_strtoupper($parObj->_getLabenames($arrayDataEditProfile,'myImage','name'),'UTF-8');?> </div>
          <div class="col-02"><span><input name="user_photo" type="file" class="filefieldEdit" />
				  ...
				  <? if($currentImage != ""){?>
				  <a href="javascript:void(0)" class="col-02" onClick="openNewWindow('<?=ROOT_FOLDER?>uploads/users/<?=$currentImage?>','windowname',<?=($imageDetails[0]+100);?>,<?=($imageDetails[1]+50);?>)"></a>
				  <? }?></span>
          ...
          <div class="bloks">
             <h3><b><?=mb_strtoupper($parObj->_getLabenames($arrayDataProfile,'newOptionalInfoTxt','name'),'UTF-8');?></b></h3>
                <div class="rows">
                ...
                <div class="rows"><? if($err14 == 1){ ?><div class="prf_err"><img src="<?=ROOT_FOLDER?>images/help_editprfl.png" class="Q-mark" onMouseOver="tooltip('<?=$weight?>');" onMouseOut="exit();"><? }?>
                   <div class="col-01"><?=mb_strtoupper($parObj->_getLabenames($arrayData,'weight','name'),'UTF-8');?>:   </div>
                   <div class="col-02">
                  <div class="split">   <input name="user_weight_value" type="text" class="tfl_small" value="<?=stripslashes($_POST[user_weight_value])?>"> </div>
                   <div class="split">
                   <div class="selet3"> <select name="user_weight_unit"> 
					    <?
						foreach($weightUnits as $key => $data){
						?>
					   <option  value="<?=$key?>" <? if($_POST['user_weight_unit'] == $key) echo 'selected';?>><?=mb_strtoupper($data,'UTF-8')?></option>
					    <?
						} ?>
					   </select></div></div>
                   </div><? if($err14 == 1){ ?> </div><?}?>
                ...
                <div class="rows"><? if($err15 == 1){ ?><div class="prf_err"><img src="<?=ROOT_FOLDER?>images/help_editprfl.png" class="Q-mark" onMouseOver="tooltip('<?=$height?>');" onMouseOut="exit();"><? }?>
                   <div class="col-01" <? if($err15 == 1){ ?> class="errorMessageBgCenter"<? }?>><?=mb_strtoupper($parObj->_getLabenames($arrayData,'height','name'),'UTF-8');?>    </div>
                   <div class="col-02">
                  <div class="split"> <input name="user_height_value" type="text" class="tfl_small" value="<?=stripslashes($_POST[user_height_value])?>"></div>
                    <div class="split">
                   <div class="selet3"> <select name="user_height_unit"> 
						 <?
						foreach($heightUnits as $key => $data){
						?>
						<option value="<?=$key?>" <? if($_POST['user_height_unit'] == $key) echo 'selected';?>><?=mb_strtoupper($data,'UTF-8')?></option>  <? }?></select></div></div>
                   </div>
                </div><? if($err15 == 1){ ?> </div><?}?>
...
foreach($optionMenus as $key => $data)
 ...
 3 boxes code-->
          <p ><input type="hidden" name="useremail" id="useremail" value="<?=stripslashes($userDetail['user_alt_email']);?>" />
				<input name="update" id="update" type="submit" value="<?=mb_strtoupper($parObj->_getLabenames($arrayDataEditProfile,'update','name'),'UTF-8');?>" class="btn_orng" />
				</div>
				<div class="bils">
            <?if($pdf == 1){?><a href="myInvoices.php"> <h3><?=mb_strtoupper($parObj->_getLabenames($arrayDataPgm,'invoice','name'),'UTF-8');?></a></h3><?}?>
             <h3 class="title2"><?=mb_strtoupper($parObj->_getLabenames($arrayDataPgm,'newHstryTxt','name'),'UTF-8');?></h3>
              ...
              <?php 
			   ...
			   { 
			   ?>
                <a href="<?=ROOT_JWPATH?>historical.php">
                <?=mb_strtoupper($parObj->_getLabenames($arrayDataPgm,'myhistorytext','name'),'UTF-8');?>
                </a>
                <?php
			   } 
   ...
   <script type="text/javascript" src="<?=ROOT_FOLDER?>crop/assets/js/jquery.min.js"></script>
  <script type="text/javascript" src="<?=ROOT_FOLDER?>crop/assets/js/bootstrap.min.js"></script>
  <script type="text/javascript" src="<?=ROOT_FOLDER?>crop/assets/js/cropper.min.js"></script>
   <script type="text/javascript" src="<?=ROOT_FOLDER?>crop/assets/js/canvas-to-blob.min.js"></script>
  <script type="text/javascript" src="<?=ROOT_FOLDER?>crop/assets/js/main.js"></script>
