...
<span class="fb-btn"><?php echo $contentObj->_getLabenames($fbLoginAry,'fb_label','name'); ?> </span>
...
?>
    <div id="fb-root" ></div>
       ...
       <section class="pop fb-log-pop-up" id="fbLoginPopup" >
            <div class="popbox">

        <table width="100%" border="0" cellspacing="2" cellpadding="0" >
		  <tr>
			<td align="center"> <div ><b><?php echo $popupMsg; ?></b></div><br></td>
		  </tr>
		  <tr>
			<td align="center">

           <input type="button"  onClick="getokid();" id="okid" name="renewSubscriptionIdBtn" class="btn_pop btn_pop_up_fb ease" value="<?php echo $popupYes; ?>">
          <input type="button" onClick="getcancelid();" id="cancelid" name="renewSubscriptionIdBtn" class="btn_pop btn_pop_up_fb ease" value="<?php echo $popupNo; ?>"></td>
		  </tr>
		</table>
         </div> </section>
       <!--pop up ends-->
...
-->
    <script type="text/javascript">
		...
		$(document).ready(function(){
			...
			//gg	$('#fbLoginPopup').css('top', '20px !important');
			<?php } ?>
			//alert(1);
			<?php if($userRegConfirm){ ?>
		 $('.pop').bPopup({
			...
			$('.fb-btn').unbind('click').click(function() { 
			  	getLoginFB();
			});
			...
			function getLoginFB(){ //
				FB.login(function(response){ 
					if (response.authResponse){ 
						if (response.authResponse){
							var access_token = response.authResponse.accessToken;
							var uid = response.authResponse.userID;//alert(uid);//return false;
							window.location =	"<?php echo JIWOK_URL; ?><?php echo $fblancode.$targt; ?>?fbLogin=1";
						}
					}
				},{scope:'email,user_birthday'});
