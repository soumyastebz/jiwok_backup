...
$retryMsg	=	$parObj->_getLabenames($arrayPayData,'paymentretrypop','name');
//$retryMsg	=	normalze(nl2br(html_entity_decode($retryMsg, ENT_QUOTES, "utf-8")));
//$retryMsg	="jjjjj";
$checkretry	=	checkStripeRetry($userid);
global $onloadglobal;
//if($userid == "251278"){

if((count($checkretry)>0) && ($checkretry['attempt_count'] > 0) && (trim($_REQUEST['conf']) != ""))
{ 
	
	$onloadglobal	= "displayRetryPopMessage();";
	
}
elseif(trim(stripslashes($program['program_expdate']))!= "" && $today >= trim(stripslashes($program['program_expdate'])) && trim($_REQUEST['conf']) != "") { 
      ...
      <ul class="rslides callbacks callbacks1" id="slider4">
                ...
                <?=mb_strtoupper($parObj->_getLabenames($arrayData,'gotoworkout','name'),'UTF-8');?>
					 ...
					 <?php
				 	 ...
				 	 <a href="referrel_system.php" class="link referel">
                        <?=mb_strtoupper($parObj->_getLabenames($arrayDataProfile,'refUrlTxt','name'),'UTF-8')?>
                        </a>
                        <?php
				} }
				 else
...
-->
 ...
 <div class="frame_inner">
       ...
       <?php   $image_user 	= 'uploads/users/'.$user_photo;
				...
				{
					if(file_exists('../www.jiwok.com/uploads/users/'.$user_photo) && (!empty($user_photo)))
					 {
					$image_user = 'http://en.jiwok.com/uploads/users/'.$user_photo;				
...
<?php
function checkStripeRetry($userId)
{
	$selectQuery1stripe	=  "select  * from stripe_payment where user_id ='".$userId."' and status='ACTIVE'";
	$striperesult		= 	$GLOBALS['db']->getrow($selectQuery1stripe,DB_FETCHMODE_ASSOC);
	return $striperesult;
}
include("footer.php"); 
?>
