...
<!--for search error popup-->
    ...
    <ul>
      ...
      <?php //echo "<pre>";print_r($topSrch);exit;
		...
		//	if($_SESSION['user']['userId']==107944){
			
			 $web_link   = $topData['web_link'];
			 $cat_flex_id= end(explode("-",$web_link));
			 $topImages	=$searchObj->getTopImages($lanId,$cat_flex_id);
			 $titlecat   =$topImages['title']	;
		    	if($topImages['category_image'] != '')
				{ 
					$imgPath = "admin/crop/assets/img/";
					$image = $objGen->_output(trim($topImages['category_image']));
					$image_new	=	$imgPath.$image;
					if(file_exists($image_new) && (!empty($image)))
					{
						$topImage	=	$imgPath.$image;
					}
					else
					{
						$topImage	= "images/dummy.jpg";
					} 
				}
				else
				{
	...
	?>
      <li>        
        <a href="<?php echo $topData['web_link']; ?>" OnClick="location.href='<?php echo $topData['web_link']; ?>'" target="_blank">
        <figure>
                 <img class="corner" data-lazy-src="<?=ROOT_FOLDER?>images/corner-4.png" alt="jiwok">
                 ...
                 <img data-lazy-src="<?=ROOT_FOLDER.$topImage?>" alt="<?php echo $titlecat;?>"/>
                 </div>
             </figure>
        </a>
