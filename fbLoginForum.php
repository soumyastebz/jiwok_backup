<?php
include_once('/home/sites_web/client/newdesign.back/tools/fbconnect/src/facebook.php');
include_once('/home/sites_web/client/newdesign.back/includes/classes/class.fbLogin.php');

$facebook = new Facebook(array(
  'appId'  => '114590195561406',
  'secret' => 'cd16501df9d0d7c15495db49d2704770',
));
$fbLogin		=	new fbLogin();
...
if (($user_profile)&&($_REQUEST["fbLogin"]=="1")) { // Check whether the user fb user set and user tried to login 
	if($user_profile["email"]!=""){	// Check whether email id is null
		...
		if($userId){	// User exists
			...
			$fbLogin->setUserData($user_profile,$facebook->accessToken);
			header('Location:http://www.beta.jiwok.com/userreg2.php');
			exit;
		}
	}
 } else {
	if($_REQUEST["fbLogin"]=="1"){ 
		header("location:http://www.beta.jiwok.com/".$targt."?fbLogin=1");
		exit;
	} 
    $showBtn	=	true;
}
