...
switch($action){ 
	...
	case "fb_add_token":
		...
		$fb->api("/".$uid."/feed", "post", $fbMessage);
	...
	case "tweet_auth":
		if(isset($_SESSION["oauth_token"])){
			...
			$token_credentials 	= 	$connection->getAccessToken($_SESSION['oauth_verifier']);
			$ction 					= 	new TwitterOAuth(TWITTER_CONSUMER_KEY, TWITTER_CONSUMER_SECRET,$token_credentials['oauth_token'],$token_credentials['oauth_token_secret']);
			$userToken				=	$referal->getUserToken();
			...
			if ($httpCode == 200){
				...
				if($referal->checkUser()){
					$updateArray	=	array("user_id"=>$userId,"twitter_oauth_token"=>$token_credentials['oauth_token'],"twitter_oauth_token_secret"=>$token_credentials['oauth_token_secret'],"twitter_status"=>"1");	
					...
					$insertDbValues	=	$userId.",'".$token_credentials['oauth_token']."','".$token_credentials['oauth_token_secret']."','".$userToken."',1";
					$referal->dbInsertSingle("jiwok_share_social",$insertDbFields,$insertDbValues);
				}
	...
	case "open_invite_contacts":
		...
		if(($_POST['emailid']=="")||($_POST['password']=="")){echo "ll";exit;
			$result	=	array("mailTxt"=>"","mailData"=>"<p class='err'>".$parObj->_getLabenames($arrayData,'refEmailMiss','name')."</p>");
			echo json_encode($result);
			exit;
		}
		if(!$inviter->login($_POST['emailid'],$_POST['password'])){ echo "pp";print_r($_POST);exit;
  	...
  	case "send_mail": echo "ff";exit;
		...
		if(count($validEmail) > 0){
			$resultMail	=	$referal->sendmail($validEmail,$userName,$messageTpl,$userToken);
		}
