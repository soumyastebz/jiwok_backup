...
//code for press magazine api token generation
if(($lanId!=5) && ($_SESSION['user']['userId'] != "") && (($_SESSION['user']['userId'] == "107944")||($_SESSION['user']['userId'] == "288977") ||($_SESSION['user']['userId'] == "82590"))){ 
 include_once('includes/classes/class.pressMagazine.php');
 $pressApi		=	new pressMagazine();
 $userExist		=	$pressApi ->checkUSerExistToken($_SESSION['user']['userId']);

 if($userExist	== 0)
 {
 	$token			=	$pressApi ->generateToken($_SESSION['user']['userId']);
	$instoken		=	$pressApi ->insertToken($_SESSION['user']['userId'],$token);
 }
 else
 {
	 $token			=	 $userExist['user_token'];
...
if(($page_name != 'userreg1.php') && ($page_name != 'userreg2.php') &&($pg_name!='login_failed'))
{
     ...
     <footer>
          <div class="frame-footer-sec">
             <nav class="col-01">
				<span class="logo">
				<!--<a class="logo" href="#">-->
				<img src="<?=ROOT_FOLDER?>images/logo-footer.png" alt="Jiwok">
                <?php
                /*if($lanId	!=	5)
				{
                ...
                <ul class="footnav_01">
                 <li><a href="<?=ROOT_JWPATH?>press_testimonial_details.php?select=t"><?=$parObj->_getLabenames($arrayDataFooter,'testimonials','name');?></a></li>
                   <li><a href="<?=ROOT_JWPATH?>press_testimonial_details.php?select=p"><?=$parObj->_getLabenames($arrayDataFooter,'talkabout','name');?></a></li>
...
<br>
              ...
              <nav class="col-02">
				    <span class="h-style"><?=strtoupper($parObj->_getLabenames($arrayDataFooter,'whatbrings','name'));?></span>
                    <ul class="footnav_02">
                     <?php if($lanId!=5) { ?>
		...
		<li><a href="<?=ROOT_JWPATH?><?=$goalbeginrun;?>"><?=$parObj->_getLabenames($arrayDataFooter,'beginbyrun','name');?></a></li>
         ...
         <li><a href="<?=ROOT_JWPATH?><?=$goalrun;?>"><?=$parObj->_getLabenames($arrayDataFooter,'runfaster','name');?></a></li>
          <?php } ?>
                    ...
                    <span class="h-style"><?=strtoupper($parObj->_getLabenames($arrayDataFooter,'coaches','name'));?></span>
                    <ul class="footnav_02">
         ...
         <li><a href="<?=ROOT_JWPATH?><?php echo $coachUrl; ?>"><?=$parObj->_getLabenames($arrayDataFooter,'running','name');?></a></li>
              ...
              <nav class="col-03">
                <span class="h-style"><?=strtoupper($parObj->_getLabenames($arrayDataFooter,'workouttailor','name'));?></span>
                <div class="clear"></div>
                <div class="colums">
                <ul class="footnav_02">                
				<?php         
                ...
                foreach($arrayLinks as $arrayKey => $arrayLink)        
                    {        
						...
						{       
                            $link 		 =	str_replace("https://www.jiwok.com","http://10.0.0.8/jiwokv3",$link);
						}
              ...
              <li> <a target="_blank" href="<?php echo $link;?>"><?php echo $footerName;?></a></li>
...

				{
	   				...
	   				echo ((++$arrayKey%14)==0) ? '</ul></div><div class="colums"><ul class="footnav_02">':'';
...
<p>Installez notre application Iphone et Android pour suivre les séances de coaching Jiwok : séance running, jogging, course à pied, natation, marche, elliptique, vélo, tapis de course, marathon, semi marathon, 10 km, trail, débutant en sport, perte de poids</p>
 ...
 <?php /*?>  <li> <a href="<?=JIWOK_URL?>contact-us"><?=$parObj->_getLabenames($arrayDataFooter,'contact','name');?></a></li><?php */?>
   <li> <a href="<?=JIWOK_URL?>contact-us"><?=$parObj->_getLabenames($arrayDataFooter,'contact','name');?></a></li>

  
   <li>|</li>

   <?php /*?><li> <a href="<?=JIWOK_URL?>terms-and-conditions"><?=$parObj->_getLabenames($arrayDataFooter,'terms','name');?></a></li><?php */?>
   <li> <a href="<?=JIWOK_URL?>terms-and-conditions"><?=$parObj->_getLabenames($arrayDataFooter,'terms','name');?></a></li>

   <li>|</li>

   <li> <a href="<?=JIWOK_URL?>press_testimonial_details.php?select=p"><?=$parObj->_getLabenames($arrayDataFooter,'press','name');?></a></li>

   <li>|</li>

   <li> <a href="<?=JIWOK_URL?>jobs"><?=$parObj->_getLabenames($arrayDataFooter,'job','name');?></a></li>

   <li>|</li>
   <?php if($lanId!=5){?> 
  <?php /*?> <li> <a href="<?=JIWOK_URL?>partners"><?=$parObj->_getLabenames($arrayDataFooter,'partnersbottom','name');?></a></li><?php */?>
   ...
   <?php
   if(($_SESSION['user']['userId'] == "107944") ||($_SESSION['user']['userId'] == "288977")||($_SESSION['user']['userId'] == "82590")){
    if(($lanId!=5) && ($_SESSION['user']['userId'] != "")){?>
   <li>|</li>
   <li> <a href=" http://jiwok.kioskpress.fr/#/?token=<?=$token?>"><?="kioskpress";?></a></li>
   <?php }
   }?>
   </div>
   </ul>  
