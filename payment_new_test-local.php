...
if($_POST['stripePayment'] == 1){ //stripe pop up after action
	...
	if($striperesult1[0]['attempt_count']  != 0)
	{
...
if(trim($_POST['user_discount'])!="" ):
	...
	if($errorMsg == 0):	
		...
		header("Location:payment_new_test.php?payment_plan=".$_REQUEST['payment_plan']);
		exit;
	endif;		
endif;
...
?>
 <section class="payment-new">         
    ...
    <ul>
        <li><?=$parObj->_getLabenames($arrayData,'searchPath','name');?></li>
        <li><a href="<?= $homeUrl ?>"><?=$parObj->_getLabenames($arrayData,'homeName','name');?></a></li>
        <li>&gt;</li>
        <li><a href="#" class="select"><?=$parObj->_getLabenames($arrayData,'myaccount','name');?></a> </li>
</ul>
...
<section class="payment-boxcontent">
   <article>
     ...
     <form method="post" action="payment_new_test.php" id="plan" onSubmit="return validatePaymentNew(<?php echo $languageid;?>);" name="plan">  
     <?php
	  if($lanId	==	5)
	  {
		   if(sizeof($plan12) > 0)
	  ...
	  if(((sizeof($result1) == 0) ||(sizeof($striperesult1) == 0)  ) &&	(sizeof($result3) > 0) &&	($result4[0]['count'] > 0) && !isset($_POST['make_payment']) && (sizeof($result2) == 0)&& !$discUser	&&	!isset($_POST['oxysubmit']) )
{
	...
	else
	{
	  ...
	  ?>
        <td align="center"><a id="okIdCmnPaymentAlert"><input class="btn_pop ease" onclick="close_popup_confirm_alert('nnnn');"  name="renewSubscriptionIdBtn" type="button" value="Ok" /></a></td>
      </tr>
    </table>
    <div class="clear"></div>
  </div>
...
<script>
	...
	$(document).ready(function(){		
		...
		{			
			...
			jpopup = $('.pop_paymantCmnAlertMsg').bPopup({speed: 500,positionStyle: 'absolute', });//gg
		}
...
<!--==============-->
<!--  pop up  msg for expiry date greater than today users -->

<div class="pop_paymantCmnAlertMsg" id="paymantexpiryDateAlertMsg" style="display:none;position:fixed;z-index:100000;">
  <a href="javascript:void(0)" title="close" id="closePaymantCmnAlertMsg" style="display: inline;"></a>
  <div class="popbox_paymantCmnAlertMsg">  
    <table width="100%" border="0" cellspacing="2" cellpadding="0" class="content">
      <tr>
        <td align="center" class="pop-box" ><div id="alertMsgExpirydatePayment"></div></td>
      </tr>
      <tr>
      <?php
	  if(((sizeof($result1) == 0) ||(sizeof($striperesult1) == 0)  ) &&	(sizeof($result3) > 0) &&	($result4[0]['count'] > 0) && !isset($_POST['make_payment']) && (sizeof($result2) == 0)&& !$discUser	&&	!isset($_POST['oxysubmit']) )
...
<script>
	...
	function close_popup_confirm_alert(expirydatemsg){
		...
		document.getElementById("alertMsgExpirydatePayment").innerHTML	=	msg;
			jpopup = $('#paymantexpiryDateAlertMsg').bPopup({	     
               speed: 200,
               positionStyle: 'absolute',
           }); 
document.getElementById("paymantexpiryDateAlertMsg").style.display="block";
...
function getPaymentButton($planId,$count_result,$userlang,$name_tag,$attempt_count)
{
	...
	$form_content	='<form method="post" action="payment_new_test.php" id="plan"  name="plan">';
  ...
  var handler = StripeCheckout.configure({
    key: 'pk_test_p5CtnbbT24RqAM7Wt4DCFUwS',
