...
{
	header("location:https://en.jiwok.com".$_SERVER['REQUEST_URI'],true,301);
}
...
if($_REQUEST['forum'] == 1 && $_REQUEST['returnUrl'])
	{
		...
		$curPage = "https://www.jiwok.com/forum/test.php";
	}
...
if((isset($_SESSION['jiwokforum']['email']) && isset($_SESSION['user']['userId'])) || isset($_SESSION['jiwokforum']['user'])) 
	{  
		$forum_path = "https://www.jiwok.com/forum/index.php?sid=".$_SESSION['jiwokforum']['sid'];  
	}
elseif(!isset($_SESSION['jiwokforum']['email']) && isset($_SESSION['user']['userId'])) 
	...
	{ 
		$forum_path = "https://www.jiwok.com/forum/index.php?open=1";  
	}
...
{
	$fb_image	= "https://www.jiwok.com/marathon/images/Jiwok_Marathon.png";
	$fb_link	= "https://www.jiwok.com/Marathon-Paris-seance-course-jiwok";
...
<body <?php if(($page_name == 'userreg1.php') || ($page_name == 'userreg2.php') || ($pg_name =='login_failed') || ($pg_name =='login_failed1')) { ?> id="entry" <?php } if($pg_name=='userArea')
 ...
 // }
  ...
  ?>
   ...
   else 
   {
		...
		$paidlink			=	"SELECT	payment_status,payment_expdate,payment_id FROM payment WHERE payment_userid =".$userId." AND payment_status = '1' ORDER BY payment_id  DESC limit 0,1 ";	
		$paidlink_status	=	$GLOBALS['db']->getRow($paidlink, DB_FETCHMODE_ASSOC);
		/*================================code for kioskpress,referal link  starts===================================*/
		...
		$showRefUrl			=	false;
		$showRefUrlstatus	=   false;
		//kioskpress page and Ref link 
		$showRefUrl			=	$objPgm1->getSubStsUser($userId);
		$AccountExpDate 	= $objPgm1->_findAccountExpireDate($userId);	
				
		if($AccountExpDate >= date('Y-m-d'))
		{
			$showRefUrlstatus = true;
     ...
     <ul class="user"> 
		 <?php 
		 if(($showRefUrl) && ($showRefUrlstatus ))		// to kioskpress original page  
		 { ?>
		<li><a href="http://jiwok.kioskpress.fr/#/?token=<?=$token?>" target="_blank" class="magazine_link">Vos magazines gratuits&nbsp;&nbsp;</a></li>
	<?php } 
	else if (($showRefUrl) && ($showRefUrlstatus == false  ))	{ // to nonactive page ?>
    <li><a href="http://10.0.0.8/jiwokv3/kioskipress.php" target="_blank" class="magazine_link">Vos magazines gratuits&nbsp;&nbsp;</a></li>
    <?php }
	/*================================code for kioskpress ends===================================*/
	 ?>
...
<section class="pop"> 
             <div class="login">
             <span class="login-h2"><?=$parObj->_getLabenames($arrayDataHead,'login','name');?></span>
