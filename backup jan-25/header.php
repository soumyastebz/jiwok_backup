...
/* For getting errors */
if($_SESSION['user']['userId'] == 162584){ // For account sanal.reubro@gmail.com
...
{
/*----add link tag during pagination in header for seo------------*/
	/*if(!($maxPage ==1))
	{
		if ($pageNum == 1)
		{
			echo '<link rel="next" href="'.$href_next.'" />';
		}
	    elseif (($pageNum > 1) && ($pageNum < $maxPage))
		{
	    	echo ' <link rel="prev" href="'.$href_prev.'"/>';
			echo '<link rel="next" href="'.$href_next.'" />';
		}
...
<?php /*?><script type="text/javascript" src="<?=ROOT_FOLDER?>resources/accordion/jquery-1.5.1.js"></script><?php */?>
<script type="text/javascript" src="<?=ROOT_FOLDER?>includes/js/general.js"></script>
<script type="text/javascript" src="<?=ROOT_FOLDER?>js/flaunt.js"></script>
...
/*?>if($pg_name!='payment_renew'){ ?>
<script type="text/javascript">
$(document).ready(function(){
// ------SET LOGO URL---------
 <?php if($pg_name!='payment_renew'){ ?>
	$('.logo').wrap('<a href="<?php echo ROOT_JWPATH.$imageLink; ?>" />');
	$('.logo-inner').wrap('<a href="<?php echo ROOT_JWPATH.$imageLink; ?>" />');
 <?php } ?>
});
...
function changeCheckboxToChecked(remember)
{
	...
	if(remember == 0)
	{
      ...
      //	floatimg.setAttribute("src", "<?=ROOT_FOLDER?>images/checkbox.png");
 ...
 // }
  ...
  ?>
   ...
   else 
   {
     ...
     <li><a href="<?=ROOT_JWPATH?>logout.php?returnUrl=<?=base64_encode($curPage);?>"><?=$parObj->_getLabenames($arrayDataHead,'logout','name');?></a></li>
     </ul>
     <?php
   }
 ?>
...
-->
<?php
	if(($page_name != 'userreg1.php') && ($page_name != 'userreg2.php') &&($pg_name!='login_failed') &&($pg_name!='login_failed1'))
			{ ?>
...
if(($page_name != 'index.php') && ($page_name != 'userreg1.php') && ($page_name != 'userreg2.php') &&($pg_name!='login_failed')&&($pg_name!='login_failed1'))
{
