...
class Categories{
    ...
    public function EditCategories($xml_string){
		...
		//$xml_string = utf8_encode($xml_string);
		$xml_obj	= simplexml_load_string($xml_string);
		//$xml_obj	= simplexml_load_string(html_entity_decode($xml_string,ENT_QUOTES, 'utf-8'));
		//$xml_obj	= new SimpleXMLElement(html_entity_decode($xml_string,ENT_QUOTES, 'utf-8'));
	...
	$myFile = "myfile.txt";
		$fh = fopen($myFile, 'a');
		$stringData = "edit categories";
		fwrite($fh, $stringData);
		$stringData = "\n";
		fwrite($fh, $stringData);
		fwrite($fh,  print_r($category, true));	
	fwrite($fh, $stringData);
		//fwrite($fh,  print_r($xml_string, true));			
		$stringData = "\n";
		fwrite($fh, $stringData);		
		fclose($fh);
	//======
...
if(isset($sub_categories[$cat_flex_id])){
	...
	$myFile = "myfile.txt";
		$fh = fopen($myFile, 'a');
		$stringData = "inside create xml";
		fwrite($fh, $stringData);
		$stringData = "\n";
		fwrite($fh, $stringData);
		//fwrite($fh,  print_r($parent_categories, true));		
		$stringData = "\n";
		fwrite($fh, $stringData);
	//	fwrite($fh,  print_r($sub_categories, true));
		//fwrite($fh, $stringData);	
		fclose($fh);
	//======
...
//======
	$myFile = "myfile.txt";
		...
		$stringData = "\n";
		fwrite($fh, $stringData);
		fwrite($fh,  print_r($categories_arr, true));	
		$stringData = "\n";
		fwrite($fh, $stringData);		
		fclose($fh);
	//======
		if(sizeof($categories_arr)>0){
			...
			$insert_query = "UPDATE `sub_category` SET $values WHERE 'flex_id' = ".$flex_id;
