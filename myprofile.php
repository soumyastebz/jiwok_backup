<?php 
session_start();//echo $_SESSION['language']['xml'];;//exit;
...
if(!is_file($image_user)) { 
	if(is_file('../www.jiwok.com/uploads/users/'.$objGen->_output($userDetail['user_photo']))) //for users using english site also
				{					
...
function unSubscrb(versionId,userId){ 
	...
	var ajaxUrl	=	"http://10.0.0.8/jiwokv3/unSubscibePayBox.php?action=unsubscribe&id="+userId;
	if(versionId==1){
		ajaxUrl	=	"http://10.0.0.8/jiwokv3/unSubscibePayBox.php?action=unsubscribeOld&id="+userId;
	}else if(versionId==2){
		ajaxUrl	=	"http://10.0.0.8/jiwokv3/unSubscibePayBox.php?action=unsubscribe&id="+userId;
	}
	else if(versionId==3){ 
	
		var jiwok_url 	=	"<?php echo JIWOK_URL ; ?>";
		//ajaxUrl	=	jiwok_url+"unSubscibePayBox.php?action=unsubscribestripe&id="+userId;
		
		ajaxUrl	=	"http://10.0.0.8/jiwokv3/unSubscibePayBox.php?action=unsubscribestripe&id="+userId; 
...
xmlhttp.onreadystatechange=function(){
  if (xmlhttp.readyState==4 && xmlhttp.status==200){
	 ...
	 // alert("response" +resTxt);return false;
		  ...
		  //--------------------------------------------------------------------------------------//
			  ...
			  if($userNewPaymentStatus	>	0)
				...
				else
				{
					...
					{//echo "4";
						?>
						<p><a href="payment_renew.php" class="title4"><?php
...
-->
        ...
        <!-----newwww-->
		 ...
		 //Condition which check whether the user has strip payment active-- code by sneha dec04
		 $isNewstripebox =false;
		 $qry1	=	"select * from stripe_payment where user_id=".$_SESSION['user']['userId']." and status='ACTIVE' order by id desc";
			//echo $qry1;
		$payboxData  = mysql_query($qry1);
		 $ccchek  = mysql_num_rows($payboxData);
		 if(mysql_num_rows($payboxData)>0){
			 $payboxRow	=	mysql_fetch_array($payboxData);
			// print_r($payboxRow);
			 if($payboxRow["status"]=="ACTIVE"){
				 $isNewstripebox	=	true;
				 $payboxUserId	=	$payboxRow["id"]; //Doubt here whether to change the code or not
			 }
		 }
		...
		//Code added by sneha dec04 for checking  whether the user has unsubdata .
		
			$qry1			=	"select * from stripe_payment where user_id=".$_SESSION['user']['userId']." order by id desc";
			$resultUnSub	=	mysql_query($qry1);
			if(mysql_num_rows($resultUnSub)>0)
			{
			$unSubData	=	mysql_fetch_array($resultUnSub);
			//print_r($unSubData);
...
//~ //-------------------------------------------------------------------------------------------------------------------------------//
             ...
             <div  id="unscbContainer">
              ...
              <script language="javascript">
			  		function test(arg){
							...
							document.getElementById('hidn_arg').value=arg;  
                ...
                <a href="javascript:void(0)" onclick="return test(2);"><font class="red"> <!--new code added by sneha-->
              <?=$parObj->_getLabenames($arrayDataProfile,'memUnsubscribeLink','name');?>
              </font></a>
              </h2>          
              <div class="btm">
              <input type="hidden" id="hidn_arg" >
              </div>
            </div>
	     <?php
		 }
		 
		 //Code added by sneha dec2014
		 else if($isNewstripebox){
			// echo "enterrr";
		 ?>
            <div  id="unscbContainer">
              <h2 class="title3" ;="" style="color:#E57200" >
              ...
              <script language="javascript">
                ...
                <a href="javascript:void(0)" onclick="return test(3);"><font class="red"> <!--new code aaded by sneha-->
              <?=$parObj->_getLabenames($arrayDataProfile,'memUnsubscribeLink','name');?>
              </font></a>
              </h2>          
              <div class="btm">
		...
		// echo $Payed; echo $unSub; echo $userNewPaymentStatus; echo $newPaymentRegisterCount; echo $dayCount;exit;
		 else if($Payed == 1 && $unSub == 0 && $userNewPaymentStatus== 0 && $ccchek>0 &&	$dayCount	>	0)
		 {
			 echo "haiiiiii unsubscribe";echo $Payed; echo "<br/>";echo $unSub;  echo "<br/>";echo $userNewPaymentStatus;  echo "<br/>";echo $newPaymentRegisterCount; echo "<br/>"; echo $dayCount;
			 ?>
      ...
      <div class="popbox">
         ...
         <hr>
          <table width="100%" border="0" cellspacing="2" cellpadding="0" class="content">
            ...
            <script language="javascript">
            	function testYes(){ 
					 var hidn=document.getElementById('hidn_arg').value;
					 //alert("jhjfdf" + hidn);
						hideUnsubscribe('unsubscribePgm1')	
						if(hidn == 2) //new code added by sneha
						{					
						unSubscrb(2,'<?php echo $payboxUserId; ?>'); 
						}
						else if(hidn == 3)
						{
						unSubscrb(3,'<?php echo $payboxUserId; ?>');	
						}
				}
            </script>
