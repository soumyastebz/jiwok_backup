<?php
    ...
    $homePage 	= 	TRUE;	
/*	echo "NNNNN<pre/>";print_r($_SESSION);
	echo "PPPP";print_r($_COOKIE);*/
	...
	if($lanId=="")	$lanId=2; //lanid=languageid
	$objGen     	= new General();
   ...
   // Redirect to logout from ticket section
	//if logut the user will be redirected to index.php
...
<div class="callbacks_container">
        <ul class="rslides" id="slider4">
         ...
         <li>
           ...
           <h2 class="caption">
		   <span>JIWOK.</span>
           <?php echo $values['slide_content_name']; ?></h2><!-- JIWOK.le bon coaching sur la bonne musique-->
...
<!-----slider closed---------------->   
   ...
   <!-- the search wizard validation and result showing -->
...
<!--------Testimonials closed---------------------->
<!--------Top 10 plans in jiwok-------------------------->
   <?php

		$topCntrCnt=0;
		$topSrch=$searchObj->getTopSearchs($lanId);//echo"<pre>";
		print_r($topSrch);//exit;
		if(count($topSrch)>0){
			
		?>
     <div  class="frame top-entry mid-wrapper">
		<span><?=$parObj->_getLabenames($arrayData,'top-10','name');?></span>
        <span id="site_top10">
    <?php
