...
//~ foreach( $workoutArray as $key => $value){
	...
	//~ 
//~ }
$workoutDates =array('2009-02-20','2009-02-26','2009-02-27','2009-02-28'); //need to delete this
$output = '';
$month  = '';//need to change this
$year   = '';//need to change this
...
$lanName="english";//need to del this
	...
	<td colspan='7'>
		...
		<tbody><tr>
			<td style='padding:2px 0 0 10px;'>
				<a href='javascript:navigate($prev,$prevy)'><img src='calander_images/arw_lft.png'></a></td>
			<td align='center'>
				<div class='calheadMonth'>
				  $name1 $year2
				</div>
			</td>
		    <td style='padding:2px 10px 0 0;'><a href='javascript:navigate($next,$nexty)'><img src='calander_images/arw_rit.png' /></a></td>
		</tr>
		</tbody></table>
	</td>
</tr>";
	...
	if($i <= ($days+($start-1)) && $i >= $start) {
		...
		if(in_array($curdate,$workoutDates))
		{
		  ...
		  if($curdate<$todayDate)
			...
			$output.="<div class='day_Tick' onclick=\"
			selectWorkout('a','$cur','$workoutFlexId','$workoutOrder');										
			htmlData('workout_pagination_cal_eng.php','p=$workoutOrder','$flexid','$pgmid');									
			htmlComment('comment_pagination_cal.php','pg=1','$pgmid','$workoutFlexId');
			topWorkOutButtonOnclick('workout_onclick.php','p=$workoutOrder','$flexid','$pgmid');\" id='$c1' ><b>";
		  elseif($curdate==$todayDate)
		  	//$output.="<div class='day_Today' onclick=\"this.className='day_Orange1'\" id='$c2'><b>";
			$output.="<div class='day_Today' onclick=\"
			selectWorkout('b','$cur','$workoutFlexId','$workoutOrder');								
			htmlData('workout_pagination_cal_eng.php','p=$workoutOrder','$flexid','$pgmid');
			htmlComment('comment_pagination_cal.php','pg=1','$pgmid','$workoutFlexId');
			topWorkOutButtonOnclick('workout_onclick.php','p=$workoutOrder','$flexid','$pgmid');\" id='$c2' ><b>";
		  elseif($curdate>$todayDate)	
		   	//$output.="<div class='day_Orange2' onclick=\"this.className='day_Orange1'\" id='$c3'><b>";
			$output.="<div class='day_Orange2' onclick=\"
			selectWorkout('c','$cur','$workoutFlexId','$workoutOrder');
			htmlData('workout_pagination_cal_eng.php','p=$workoutOrder','$flexid','$pgmid');
			htmlComment('comment_pagination_cal.php','pg=1','$pgmid','$workoutFlexId');
			topWorkOutButtonOnclick('workout_onclick.php','p=$workoutOrder','$flexid','$pgmid');\" id='$c3' ><b>";
		  else
