...
/**************************************************************************** 
   ...
   DESCRIPTION::::>>>>
   This  code used to list the all program's category.
   Admin can  edit program category's image . 
*****************************************************************************/
	...
	*/
	...
	$heading = "Category's Image Management";
	$heading_title = "Category's title";
	...
	/* $cleanData	=	str_replace("'",'\\\\\\\\\'',trim($_REQUEST['keyword']));
		...
		$errMsg = "Special characters are not allowed";
			...
			$searchQuery	= " AND `category_name` LIKE '%".$cleanData."%'";
			}*/
 $searchQuery	= "AND `category_name` LIKE  '%".trim(mysql_real_escape_string($_REQUEST['keyword']))."%' ";	
	}
	...
	$totalRecs = $objImg->_getTotalCount_cat($searchQuery,$lanId);
	if($totalRecs <= 0)
		$errMsg = "No Records";
	...
	if($_REQUEST['pageNo']){
		...
		$result =  $objImg->_showPage_cat($totalRecs,$_REQUEST['pageNo'],$_REQUEST['maxrows'],$field,$type,$searchQuery);
	}
	else{
...
<? if($_POST['update1']){
	for($i=0;$i<100;$i++){ 
		 ...
		 //~ if(!empty($image_name))
		 //~ {
		$sql = "select img_new from category_image  where flex_id=$prgm_id";
        $result = $GLOBALS['db']->getAll($sql, DB_FETCHMODE_ASSOC);
          if(count($result)>0)
          { 
			  $sql = "update category_image set img_new='".$image_name."' where flex_id=$prgm_id";
					
		   $result = $GLOBALS['db']->query($sql);
		}		 
			 else
  {    
	   $query = "INSERT INTO category_image (flex_id,img_new) VALUES ($prgm_id,'$image_name')";
 	   $result =  $GLOBALS['db']->query($query);
          ...
          }header('Location: http://beta.jiwok.com/admin/program_cat_img.php');
	  }?>
...
<tr> 
              ...
              <tr> 
			   ...
			   <form name="frm_cat_images" action="program_cat_img.php" method="post">
				  ...
				  <tr>
					...
					<TD align="left">
						...
						<?  if($objGen->_output($_REQUEST['keyword'])){ ?>
							<td valign="middle" width="50"><a href="program_cat_img.php?maxrows=<?=$_REQUEST['maxrows']?>"><img src="images/list.gif" alt="Listing Record">&nbsp;List </a></td>
						<? }else{ ?>
				  ...
				  <TABLE class="listTableStyle" cellSpacing=1 cellPadding=2 width="553">
				   <TBODY> 
					   <TR class="tableHeaderColor">
						...
						<a href="program_cat_img.php?field=category_name&type=asc&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&pageNo=<?=$_REQUEST['pageNo']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>"><img src="images/up.gif" border="0" alt="Ascending Sort">&nbsp;</a>
						<a href="program_cat_img.php?field=category_name&type=desc&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&pageNo=<?=$_REQUEST['pageNo']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>"><img src="images/down.gif" border="0" alt="Descending Sort"></a>
				
						</TD>
						<TD width="9%" align="center" >image name</TD>
						...
						?>
							...
							<?if($row['img_new']==''){?>
                             <TD align="center"><input type="text" name="image_<?php echo $key;?>" id="image_<?php echo $key;?>" ></TD>
                              <input type="hidden" name="lan_id_<?php echo $key;?>" id="lan_id_<?php echo $key;?>" value="<?php echo $lanId; ?>">
                               <input type="hidden" name="flx_id_<?php echo $key;?>" id="flx_id_<?php echo $key;?>" value="<?php echo $row['flex_id']; ?>"><?} else{?>
								 <TD><input type="text" name="image_<?php echo $key;?>" id="image_<?php echo $key;?>" value="<?=$row['img_new'];?>"></TD>
								  <input type="hidden" name="lan_id_<?php echo $key;?>" id="lan_id_<?php echo $key;?>" value="<?php echo $lanId; ?>">
								  <input type="hidden" name="flx_id_<?php echo $key;?>" id="flx_id_<?php echo $key;?>" value="<?php echo $row['flex_id']; ?>"><?}?>
								
								<TD align="center"><?if($row['img_new']==''){?>
...
{  
	...
	var lan_id = document.getElementById('lan_id_'+res).value ;
	var flx_id = document.getElementById('flx_id_'+res).value ;
	var langId_new = document.getElementById('langId_new').value ;
	    window.location ='../admin/crop/crop_category.php?image='+image+'&lan_id='+lan_id+'&flx_id='+flx_id+'&lanId='+langId_new;
	//var x = document.getElementById("image").value;
				...
				<table cellspacing=0 cellpadding=0 width=553 border=0 class="topColor">
					...
					<tr>
						...
						<a href="program_cat_img.php?pageNo=1&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>">
						<img src="images/mail-arrowfg.gif" border=0 width="20" height="20" align="absmiddle" alt="First Page"></a>
						<a href="program_cat_img.php?pageNo=<?=$prev?>&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>">
						...
						<select name="pageNo" class="paragraph"  onChange="form.submit()">
							...
							?>
							 ...
							 of <?=$noOfPage?>]
							 <a href="program_cat_img.php?pageNo=<?=$next?>&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&field=<?=$_REQUEST['field']?>&maxrows=<?=$_REQUEST['maxrows']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>">
							<img src="images/mail-arrowrg.gif" border=0 width="20" height="20" align="absmiddle" alt="Next Page"></a>
							<a href="program_cat_img.php?pageNo=<?=$noOfPage?>&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>">
