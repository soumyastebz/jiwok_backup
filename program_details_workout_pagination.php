...
if ($pageNum > 1)
	{
		...
		$workoutfirst = $workout_flex_ids[1];
			$prev = "<div class='page_btn'><strong><a href=\"javascript:;\" class='btn rit' onclick=\"javascript:htmlData('program_details_workout_pagination.php','p=$page','".urlencode($pgm_flexid)."','$pgmid');htmlComment('comment_pagination.php','pg=1','$pgmid','$workoutprev');\">&lt; ".$parObj->_getLabenames($arrayData,'prevsession','name')."</a></strong></div>";
...
if($workout_cnt > 0)
	{ 
		$data_pgm 	  	 	= $objPgm->_displayTrainingProgram($pgmid,$lanId);
		if($data_pgm['program_author'] == 'Stephanie')
		{
		...
		//echo $data_pgm['program_author'];exit;
        ...
        $result	 .=	"<figure><img src='".ROOT_FOLDER."images/".$coachImage."' alt='image'></figure>";
        $result	 .=	"</div>";
		...
		$result.=	'<div  class="force-overflownew overflowauto" id="style-5" itemprop="description">';	
		
		$result.=	'<h2 itemprop="alternativeHeadline">'.$workout_title.'</h2>';
		$result.=	$data;	
		$result.=	'</div>';	
		$result.=	'<div class="separator-details">';
		$result.=	'<h2 itemprop="alternativeHeadline"><strong>'.$parObj->_getLabenames($arrayData,'provide_w','name').'</strong></h2>';
		$result.=	'<div itemprop="guideline"><strong>'.$provides.'</strong></div>';
		$result.=	$qstnpop.$prev.$next;
		$result.=	'</div>';
        ...
        $programType 	= $trainingTypeFlexId[trim($data_pgm['training_type_flex_id'])];
 ...
 $result	 .= 	'<div class="new_sub_but1"><a href="javascript:;" class="btn-sign" onclick=window.location.href="login_failed.php?fromPgm=1&returnUrl='.$loginUrl.'&msg='.base64_encode(3).'"  name="subscribe" id="subscribe" rel="nofollow">'.mb_strtoupper($parObj->_getLabenames($arrayData,'subscribe_unlog','name'),'UTF-8').'</a></div>';
	 		...
	 		elseif($programType=="program")
				...
				{ 
	  			if($workout_cnt>0)
					...
					$result	 .= '<a href="javascript:;" class="new_sub_but2"  name="subscribe" id="subscribe" onclick="showSessionMessage();" rel="nofollow">'.$parObj->_getLabenames($arrayData,'subscribe','name').'</a>';
					...
					$result	 .='<a href="javascript:;" class="new_sub_but2"  name="subscribe" id="subscribe" onclick="document.tt.submit();" rel="nofollow">'.$parObj->_getLabenames($arrayData,'subscribe','name').'</a>';
					$result	 .='<input name="subscribe" type="hidden" id="subscribe" value="'.mb_strtoupper($parObj->_getLabenames($arrayData,'subscribe','name'),'UTF-8').'/>';
					endif;
