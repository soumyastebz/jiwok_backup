...
$datatext 		.= '<section class="jw_comments mid-wrapper">';
 $datatext 		.= '<span  itemscope itemtype="https://schema.org/Comment" >';
 $datatext 		.= '<h2 itemprop="headline" >'.$parObj->_getLabenames($arrayData,'jiwokcomments','name').'</h2>';
		...
		if($userImage != "")
		{
			...
			//========================================
			 ...
			 $datatext 		.= '<article class="colums">';
          	 $datatext 		.= '<figure itemprop="image"> <img src="'.ROOT_FOLDER.$userimg.'" alt="user"> </figure>';
			 $datatext 		.= '<h2 itemprop="creator" >'.$objGen->_output(trim($row['firstname'])).'</h2>';
			 $datatext 		.= '<p itemprop="comment" >'.$objGen->_output(trim($row['description'])).'</p>';
			 $datatext 	    .= '</article>';
...
$query = "SELECT COUNT(`fdbk`.`program_id`) as `dt` 
	FROM 
	`feedback` as `fdbk` JOIN `user_master` as `um` USING(`user_id`)
	WHERE `fdbk`.`program_id` = '$pgmid' AND `um`.`user_status`  = '1'
	AND `fdbk`.`public_status` ='0' AND `fdbk`.`lang_id` ='".$lanId."'  ORDER BY `fdbk`.`feedback_id` DESC";
...
*/
	...
	if($numrows>0){
		if($numrows>3)
	  	$datatext 		.= '<nav class="paging"><ul id=\"pagination\">'.$prev.$nav.$next.'</li></ul></nav>';
        } 
        $datatext 		.=  '</div>';
        $datatext 	    .= '</span>';
		$datatext 		.=  '</section>';
